...
module argmin_helper#(
  ...
  for (k = 0; k < NUM_INP; k += 2) begin : node
    if (k+2 > NUM_INP) begin
      // This will be satisfied iff NUM_INP is odd, and we are at the end of the list. If
      // that's the case, generate pass-through flip-flops instead of the argmin
      // stage.
      dff#(.WIDTH(WIDTH)) val(clk, rst, inp_word[k], outp_word[k/2]);
      dff#(.WIDTH(ADDR_WIDTH)) addr(clk, rst, inp_addr_word[k], outp_addr_word[k/2]);
    end else begin
